		...
		if (!pcheck) {
			...
			if (c == ')')
			{
				...
				if (!tmp.empty() && !op_ids.count(tmp)) {
					// give new token proper type, declaration or call
					keyword_buffer.count(Keywords::FUNC)  ?  
			...
			/* if this function has already been called, 
			   we still want to know the nature of the 
			   character without the extra operations */
			ret = c == '(' || c == ')';
		}
		return ret;
	}
